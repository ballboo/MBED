...
Example program for using the MCP7940 library which demonstrate the use of alarms on the MCP7940N/M real time clock
chip. The library as well as the most current version of this program is available at GitHub using the address 
...
The MCP7940 library uses the standard SPI Wire library for communications with the RTC chip and has also used the 
class definitions of the standard RTClib library from Adafruit/Jeelabs.\n\n
The data sheet for the MCP7940M is located at http://ww1.microchip.com/downloads/en/DeviceDoc/20002292B.pdf. The 
MCP7940N has extra functionality revolving around battery backup but this library's functionality covers that chip 
as well.\n\n
The MCP7940 has two alarms that can be set. Either can be set to trigger once at a specific point in time or can be 
set to be a recurring alarm. When the alarm goes off it sets an alarm bit which can be read using the the library
routine getAlarmState() or via an input pin if the MCP7940's multifunction pin is attached to it.\n\n
This example program sets alarm 0 to trigger every time the RTC's seconds reads "0". Alarm 1 is set to trigger at 
a time "ALARM1_INTERVAL" seconds from the last time it was triggered.\n\n
...
void setup()
{
  ...
  #ifdef  __AVR_ATmega32U4__  // If this is a 32U4 processor, then wait 3 seconds for the serial interface to initialize
    delay(3000);
  #endif
